...
@testset "Network DC-PF with PTDF Model and implementing Dynamic Branch Ratings with reductions" begin
    ...
    test_results = Dict(
        ...
        StaticBranchBounds => [264, 0, 120, 120, 168])
    ...
    branch_formulations = [StaticBranch, StaticBranchBounds]
    n_steps = 2
    for branch_formulation in branch_formulations
        line_device_model = DeviceModel(
            Line,
            branch_formulation;
            time_series_names = Dict(
        ...
        for (ix, add_parallel_line_name) in enumerate(parallel_lines_names_to_add)
            ...
            for branch_name in branches_dlr
                ...
                for t in 1:n_steps
                    ...
                    dlr_data[ini_time] =
                        TimeArray(
                            data_ts + Day(t - 1),
                            PSY.get_rating(branch) * get_base_power(sys) * dlr_factors,
                        )
                end
            ...
            moi_tests(
                ps_model,
                test_results[branch_formulation]...,
                false,
            )
