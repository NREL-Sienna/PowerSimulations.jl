...
@testset "Renewable DCPLossLess FullDispatch" begin
    ...
    #5 Bus testing case
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re)
    ...
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re; parameters = true)
    construct_device!(op_model, :Renewable, model)
    moi_tests(op_model, true, 72, 0, 72, 0, 0, false)
    psi_checkobjfun_test(op_model, GAEVF)

    # No Forecast - No Parameters Testing
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re; forecast = false)
    construct_device!(op_model, :Renewable, model)
    moi_tests(op_model, false, 3, 0, 3, 0, 0, false)
    psi_checkobjfun_test(op_model, GAEVF)

    # No Forecast Testing
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re; parameters = true, forecast = false)
    construct_device!(op_model, :Renewable, model)
...
@testset "Renewable DCPLossLess FixedOutput" begin
    ...
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re; parameters = p)
        construct_device!(op_model, :Renewable, model)
        if p
