@testset "Renewable data misspecification" begin
    ...
    op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys14)
    @test_logs (:warn, warn_message) construct_device!(op_problem, :Renewable, model)
end
...
@testset "Renewable DCPLossLess FullDispatch" begin
    ...
    #5 Bus testing case
    op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_re)
    ...
    op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_re; use_parameters = true)
    construct_device!(op_problem, :Renewable, model)
    moi_tests(op_problem, true, 72, 0, 72, 0, 0, false)
    psi_checkobjfun_test(op_problem, GAEVF)

    # No Forecast - No Parameters Testing
    op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_re; use_forecast_data = false)
    construct_device!(op_problem, :Renewable, model)
    moi_tests(op_problem, false, 3, 3, 0, 0, 0, false)
    psi_checkobjfun_test(op_problem, GAEVF)

    # No Forecast Testing
    op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_re; use_parameters = true, use_forecast_data = false)
    construct_device!(op_problem, :Renewable, model)
...
@testset "Renewable ACPPower Full Dispatch" begin
    model = DeviceModel(PSY.RenewableDispatch, PSI.RenewableFullDispatch)
    for p in [true, false]
        op_problem = OperationsProblem(TestOptModel, ACPPowerModel, c_sys5_re; use_parameters = p)
        construct_device!(op_problem, :Renewable, model)
        if p
    ...
    # No Forecast Test
    for p in [true, false]
        op_problem = OperationsProblem(TestOptModel, ACPPowerModel, c_sys5_re; use_forecast_data = false, use_parameters = p)
        construct_device!(op_problem, :Renewable, model)
        if p
...
@testset "Renewable DCPLossLess FixedOutput" begin
    ...
    for p in [true, false]
        op_problem = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_re; use_parameters = p)
        construct_device!(op_problem, :Renewable, model)
        if p
