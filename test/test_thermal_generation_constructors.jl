@testset "ThermalGen data misspecification" begin
    ...
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_re_only)
...
@testset "Thermal UC With DC - PF" begin
    ...
    @info "5-Bus testing"
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_uc)
    ...
    op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_uc; parameters = true)
    construct_device!(op_model, :Thermal, model)
    ...
    @info "14-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys14; parameters = p)
        construct_device!(op_model, :Thermal, model)
...
@testset "Thermal UC With AC - PF" begin
    ...
    @info "5-Bus testing"
    op_model = OperationModel(TestOptModel, PM.StandardACPModel, c_sys5_uc)
    construct_device!(op_model, :Thermal, model)
    ...
    op_model = OperationModel(TestOptModel, PM.StandardACPModel, c_sys5_uc; parameters = true)
    construct_device!(op_model, :Thermal, model)
    ...
    @info "14-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.StandardACPModel, c_sys14; parameters = p)
        construct_device!(op_model, :Thermal, model)
...
@testset "Thermal Dispatch With DC - PF" begin
    ...
    @info "5-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5; parameters = p)
        construct_device!(op_model, :Thermal, model)
...
@testset "Thermal Dispatch With AC - PF" begin
    ...
    @info "5-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.StandardACPModel, c_sys5; parameters = p)
        construct_device!(op_model, :Thermal, model)
...
@testset "Thermal Ramp Limited Dispatch With DC - PF" begin
    ...
    @info "5-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.DCPPowerModel, c_sys5_uc; parameters = p)
        construct_device!(op_model, :Thermal, model)
...
@testset "Thermal Ramp Limited Dispatch With AC - PF" begin
    ...
    @info "5-Bus testing"
    for p in [true, false]
        op_model = OperationModel(TestOptModel, PM.StandardACPModel, c_sys5_uc; parameters = p)
        construct_device!(op_model, :Thermal, model)
