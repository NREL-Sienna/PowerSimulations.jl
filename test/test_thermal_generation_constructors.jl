...
@testset "Thermal UC With DC - PF" begin
    ...
    op_model = OperationModel(TestOptModel, DCPPowerModel, c_sys5_uc; parameters = true)
    construct_device!(op_model, :Thermal, model)
...
@testset "Thermal UC With AC - PF" begin
    ...
    op_model = OperationModel(TestOptModel, ACPPowerModel, c_sys5_uc; parameters = true)
    construct_device!(op_model, :Thermal, model)
...
load = PowerLoad("Bus1", true, node,nothing, 0.4, 0.9861, 1.0, 2.0)
    ...
    load_forecast_ramp = Deterministic("scalingfactor", TimeArray(DA_ramp, ramp_load))
...
load = PowerLoad("Bus1", true, node,nothing, 0.4, 0.9861, 1.0, 2.0)
    ...
    load_forecast_dur = Deterministic("scalingfactor", TimeArray(DA_dur, duration_load))
...
load = PowerLoad("Bus1", true, node,nothing, 0.4, 0.9861, 1.0, 2.0)
    ...
    cost_load = [1.3,2.1];
    load_forecast_cost = Deterministic("scalingfactor", TimeArray(DA_cost, cost_load))
...
@testset "Solving UC with CopperPlate testing SOS2 implementation" begin
    ...
    load_forecast_cost_sos  = Deterministic("scalingfactor", TimeArray(DA_cost_sos, cost_sos_load))
