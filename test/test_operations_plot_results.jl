...
function test_plots(file_path::String)
    ...
    @testset "testing plot production" begin
        PSI.bar_plot(res)
        b_path = joinpath(file_path, "bar_plot.png")
        Plots.savefig(b_path)
        PSI.stack_plot(res)
        s_path = joinpath(file_path, "stack_plot.png")
        Plots.savefig(s_path)
        list = readdir(file_path)
        @test list == ["bar_plot.png", "stack_plot.png"]
    end
