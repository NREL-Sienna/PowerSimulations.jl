...
@testset "dense axis to dataframe" begin
    ...
    four = JuMPDArray{Float64}(undef, [:a], 1:2, 1:3, 1:5)
    @test_throws ErrorException PSI.axis_array_to_dataframe(four)

    three_int = JuMPDArray{Int}(undef, [:a], 1:2, 1:3)
    fill!(three_int, 2)
    ...
    @test three_df == test_df
    four = JuMPDArray{Int}(undef, [:a], 1:2, 1:3, 1:5)
    fill!(four, 2)
