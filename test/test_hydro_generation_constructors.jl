...
@testset "Hydro DCPLossLess FixedOutput" begin
    ...
    # Parameters Testing
    op_model = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_hy; use_parameters = true)
    ...
    op_model = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_hy)
    construct_device!(op_model, :Hydro, model);
    moi_tests(op_model, false, 0, 0, 0, 0, 0, false)
    psi_checkobjfun_test(op_model, GAEVF)

    # No Forecast Testing
    op_model = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_hy; use_parameters = true, use_forecast_data = false)
    construct_device!(op_model, :Hydro, model);
    moi_tests(op_model, true, 0, 0, 0, 0, 0, false)
    psi_checkobjfun_test(op_model, GAEVF)

    # No Forecast - No Parameters Testing
    op_model = OperationsProblem(TestOptModel, DCPPowerModel, c_sys5_hy; use_forecast_data = false)
    construct_device!(op_model, :Hydro, model);
    moi_tests(op_model, false, 0, 0, 0, 0, 0, false)
    psi_checkobjfun_test(op_model, GAEVF)
