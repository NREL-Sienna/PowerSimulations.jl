@testset "Load data misspecification" begin
    ...
    @test_logs (:warn, warn_message) mock_construct_device!(op_problem, :Load, model)
...
@testset "StaticPowerLoad" begin
    ...
    for m in models, n in networks, p in param_spec
        ...
        op_problem = OperationsProblem(MockOperationProblem, n, c_sys5_il; use_parameters = p)
        mock_construct_device!(op_problem, :Load, model)
