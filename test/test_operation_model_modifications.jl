...
@testset "Operation set ref models" begin
    ...
    op_model = OperationsProblem(TestOptModel, template, c_sys5)
    set_transmission_ref!(op_model, DCPLLPowerModel)
    @test op_model.template.transmission == DCPLLPowerModel
    ...
    @test op_model.template.devices[:Generators].formulation == ThermalBasicUnitCommitment
    ...
    @test op_model.template.branches[:L].formulation == StaticLine
end
