...
function test_load_simulation(file_path::String)
    ...
    for i in 1:2
        ...
        @testset "Test verify parameter feedforward for consecutive UC to ED" begin
            ...
            for (ik, key) in enumerate(P_keys)
                ...
                raw_result = DataFrame(Arrow.read(variable_ref))
        ...
        @testset "Test verify time gap for Consecutive" begin
            ...
            for name in names
                ...
                time_1 = convert(Dates.DateTime, DataFrame(Arrow.read(time_file_path_1))[end, 1]) # first time
                time_2 = convert(Dates.DateTime, DataFrame(Arrow.read(time_file_path_2))[1, 1])
                @test time_2 == time_1
            end
        end
        ...
        @testset "Test verify initial condition feedforward for consecutive ED to UC" begin
            ...
            for (ik, key) in enumerate(ic_keys)
                ...
                for ic in initial_conditions
                    ...
                    raw_result = DataFrame(Arrow.read(variable_ref))[end, Symbol(name)] # last value of last hour
    ...
    @testset "Test verify time gap for Receding Horizon" begin
        ...
        for name in names
            ...
            time_file_path_1 = joinpath(dirname(reference_1), "time_stamp.arrow") #first line, file path
            time_file_path_2 = joinpath(dirname(reference_2), "time_stamp.arrow")
            time_1 = convert(Dates.DateTime, DataFrame(Arrow.read(time_file_path_1))[1, 1]) # first time
            time_2 = convert(Dates.DateTime, DataFrame(Arrow.read(time_file_path_2))[1, 1])
    ...
    @testset "negative test checking total sums" begin
        ...
        for name in stage_names
            ...
            Arrow.write(fake_df, _file_path)
            @test_logs(
    ...
    @testset "Simulation with Cache" begin
        ...
        @testset "Test verify initial condition update using StoredEnergy cache" begin
            ...
            for (ik, key) in enumerate(ic_keys)
                ...
                for ic in initial_conditions
                    raw_result =
                        DataFrame(Arrow.read(variable_ref))[end, Symbol(PSI.device_name(ic))] # last value of last hour
                    initial_cond = value(PSI.get_value(ic))
