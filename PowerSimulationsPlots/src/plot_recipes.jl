...
function plotly_stack_gen(stacked_gen::StackedGeneration, seriescolor::Array; kwargs...)
    ...
    for gen in 1:length(gens)
        push!(traces, PlotlyJS.scatter(;name=gens[gen],
    ...
    p = PlotlyJS.plot(traces, PlotlyJS.Layout(title = "Variables", yaxis_title = "Generation (MW)"))
...
function plotly_stack_plots(res::PSI.Results, seriescolor::Array; kwargs...)
    ...
    for (key, var) in res.variables
        ...
        p = PlotlyJS.plot(traces, PlotlyJS.Layout(title = "$key", yaxis_title = "Generation (MW)"))
        set_display && display(p)
...
function plotly_bar_gen(bar_gen::BarGeneration, seriescolor::Array; kwargs...)
    ...
    p = PlotlyJS.plot(traces, PlotlyJS.Layout(
        title = "Variables",
...
function plotly_bar_plots(res::PSI.Results, seriescolor::Array; kwargs...)
    ...
    time_span = convert(Dates.Hour, (time_range[2, 1]) - (time_range[1, 1])) * length(time_range[!, 1])
    for (key, var) in res.variables
        traces = PlotlyJS.GenericTrace{Dict{Symbol,Any}}[]
        gens = collect(names(var))
        for gen in 1:length(gens)
        ...
        p = PlotlyJS.plot(traces, PlotlyJS.Layout(title = "$key", yaxis_title = "Generation (MW)", barmode = "stack"))
        set_display && display(p)
