...
function to_array(array::JuMP.Containers.SparseAxisArray)
    ...
    for (ix, col) in enumerate(columns)
        filtered_vars = collect(filter(v -> (first(v)[[1, 2]] == col) && (last(v) != 0), array.data))
        sorted_vars = sort(filtered_vars, by = x -> x[1][3])
        vars = last.(sorted_vars)
        if !isempty(vars)
            tmp_data[Symbol(col...)] = PSI._jump_value.(vars)
            push!(final_column_names, Symbol(col...))
        end
    end
