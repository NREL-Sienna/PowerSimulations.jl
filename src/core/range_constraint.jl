...
function device_semicontinuousrange(ps_m::canonical_model, scrange_data::Array{Tuple{String,NamedTuple{(:min, :max),Tuple{Float64,Float64}}},1}, time_range::UnitRange{Int64}, cons_name::String, var_name::String, binvar_name::String)
    ...
    for t in time_range, r in scrange_data
            ...
            if r[2].min == 0.0
                ...
                ps_m.constraints["$(cons_name)_ub"][r[1], t] = @constraint(ps_m.JuMPmodel, ps_m.variables["$(var_name)"][r[1], t] <= r[2].max*ps_m.variables["$(binvar_name)"][r[1], t])
                ps_m.constraints["$(cons_name)_lb"][r[1], t] = @constraint(ps_m.JuMPmodel, ps_m.variables["$(var_name)"][r[1], t] >= 0.0)

            else

                ps_m.constraints["$(cons_name)_ub"][r[1], t] = @constraint(ps_m.JuMPmodel, ps_m.variables["$(var_name)"][r[1], t] <= r[2].max*ps_m.variables["$(binvar_name)"][r[1], t])
                ps_m.constraints["$(cons_name)_lb"][r[1], t] = @constraint(ps_m.JuMPmodel, ps_m.variables["$(var_name)"][r[1], t] >= r[2].min*ps_m.variables["$(binvar_name)"][r[1], t])
