...
function make_name(::Type{T}) where {T <: VariableType}
    error("make_name not implemented for $T")
end

function make_name(::Type{T}, ::Type{U}) where {T <: VariableType, U <: PSY.Component}
    error("make_name not implemented for $T / $U")
end

struct ActivePowerVariable <: VariableType end

function make_name(::Type{ActivePowerVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, ACTIVE_POWER)
end

struct ActivePowerInVariable <: VariableType end

function make_name(::Type{ActivePowerInVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, ACTIVE_POWER_IN)
end

struct ActivePowerOutVariable <: VariableType end

function make_name(::Type{ActivePowerOutVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, ACTIVE_POWER_OUT)
end

struct AreaMismatchVariable <: VariableType end

function make_name(::Type{AreaMismatchVariable})
    return encode_symbol(AREA_MISMATCH)
end

struct ColdStartVariable <: VariableType end

function make_name(::Type{ColdStartVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, COLD_START)
end

struct EnergyVariable <: VariableType end

function make_name(::Type{EnergyVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, ENERGY)
end

struct HotStartVariable <: VariableType end

function make_name(::Type{HotStartVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, HOT_START)
end

struct LiftVariable <: VariableType end

function make_name(::Type{LiftVariable})
    return encode_symbol(LIFT)
end

struct OnVariable <: VariableType end

function make_name(::Type{OnVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, ON)
end

struct ReactivePowerVariable <: VariableType end

function make_name(::Type{ReactivePowerVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, REACTIVE_POWER)
end

struct ReserveVariable <: VariableType end

function make_name(::Type{ReserveVariable}, ::Type{T}) where {T <: PSY.Device}
...
function make_name(::Type{ServiceRequirementVariable}, ::Type{T}) where {T <: PSY.Component}
    return encode_symbol(T, SERVICE_REQUIREMENT)
end

struct SpillageVariable <: VariableType end

function make_name(::Type{SpillageVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, SPILLAGE)
end

struct StartVariable <: VariableType end

function make_name(::Type{StartVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, START)
end

struct StopVariable <: VariableType end

function make_name(::Type{StopVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, STOP)
end

struct WarmStartVariable <: VariableType end

function make_name(::Type{WarmStartVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, WARM_START)
end

struct DeltaActivePowerUpVariable <: VariableType end

function make_name(::Type{DeltaActivePowerUpVariable}, ::Type{T}) where {T <: PSY.Device}
    return encode_symbol(T, "ΔP_up")
end

struct DeltaActivePowerDownVariable <: VariableType end

function make_name(::Type{DeltaActivePowerDownVariable}, ::Type{T}) where {T <: PSY.Device}
