...
function _reading_references(results::Dict, names::Array, stage::String, step::Array,
    ...
    for name in (names)
        ...
        results[(names[name])] = DataFrames.DataFrame()
        for (ix,time) in enumerate(step_df.Date)
            file_path = step_df[ix, :File_Path]
            var = Feather.read("$file_path")
            correct_var_length = size(1:(size(var,1) - extra_time),1)
            results[(names[name])] = vcat(results[(names[name])],var[1:correct_var_length,:]) 
        end
    end
    return results
end

function _reading_references(results::Dict, names::Array, stage::String,
                             references::DataFrames.DataFrame, extra_time::Int64)
    for name in dual
        date_df = references[stage][dual[name]]
        duals[dual[name]] = DataFrames.DataFrame()
        for (ix,time) in enumerate(date_df.Date)
