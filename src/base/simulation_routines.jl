...
function run_simulations(simulation::PowerSimulationsModel{S}, solver, ps_dict::Dict; args...) where {S<:AbstractOperationsModel}
    ...
    if simulation.model.transmission <: AbstractFlowForm
    ...
    for (step, step_ts) in sort(simulation.timeseries)
        ...
        status = :termination_status in fieldnames(tmp_model.model.moi_backend.model.optimizer) ? tmp_model.model.moi_backend.model.optimizer.termination_status : nothing
