function apply_simulation_events!(simulation::Simulation)
    ...
    for event_model in events
        if check_condition(simulation_state, event_model)
            ...
            for (event_uuid, device_type_maps) in event_model.attribute_device_map[model_name]
...
function apply_affect!(
    ...
    for (dtype, device_names) in device_type_maps
        ...
        update_system_state!(sim_state, ParameterKey(AvailableStatusParameter, dtype), device_names, event, sim_time,)
        update_system_state!(sim_state, VariableKey(ActivePowerVariable, dtype), device_names, event, sim_time,)
        update_system_state!(sim_state, VariableKey(OnVariable, dtype), device_names, event, sim_time,)
        update_decision_state!(sim_state, ParameterKey(AvailableStatusParameter, dtype), device_names, event, sim_time, em_model_store)
        update_decision_state!(sim_state, VariableKey(ActivePowerVariable, dtype), device_names, event, sim_time, em_model_store)
        update_decision_state!(sim_state, VariableKey(OnVariable, dtype), device_names, event, sim_time, em_model_store)
    end
