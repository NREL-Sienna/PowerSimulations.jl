...
function build_initial_conditions_model!(model::T) where {T <: OperationModel}
    ...
    @info "Code is about to run build_impl!() from initialization.jl"
    @show model.internal.initial_conditions_model_container
    @show typeof(model.internal.initial_conditions_model_container)
    build_impl!(
