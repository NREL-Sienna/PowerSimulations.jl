...
function powermodels_network!(ps_m::CanonicalModel,
    ...
    for t in time_range, bus in sys.buses
        pm_data["nw"]["$(t)"]["bus"]["$(bus.number)"]["pni"] = ps_m.expressions[:var_active][bus.name,t]
        pm_data["nw"]["$(t)"]["bus"]["$(bus.number)"]["qni"] = ps_m.expressions[:var_reactive][bus.name,t]
    end
...
function powermodels_network!(ps_m::CanonicalModel,
    ...
    for t in time_range, bus in sys.buses
        pm_data["nw"]["$(t)"]["bus"]["$(bus.name)"]["pni"] = ps_m.expressions[:var_active][bus.name,t]
