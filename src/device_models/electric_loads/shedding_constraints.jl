...
function activepower(m::JuMP.Model, devices::Array{L,1}, device_formulation::Type{D}, system_formulation::Type{S}, time_periods::Int64) where {L <: PowerSystems.ElectricLoad, D <: AbstractControllableLoadForm, S <: PM.AbstractPowerFormulation}
    ...
    for t in time_index, (ix, name) in enumerate(name_index)
        if name == devices[ix].name
            pmax_cl[name, t] = @constraint(m, p_cl[name, t] <= devices[ix].maxactivepower*devices[ix].scalingfactor.values[t])
        else
...
#=
function reactivepower(m::JuMP.Model, devices::Array{T,1}, time_periods::Int64) where T <: PowerSystems.ElectricLoad

    activepower(m::JuMP.Model, devices::Array{L,1}, device_formulation::Type{D}, system_formulation::Type{S}, time_periods::Int64) where {L <: PowerSystems.ElectricLoad, D <: AbstractControllableLoadForm, S <: AbstractACPowerModel}
    q_cl = m[:q_cl]
    ...
    for t in time_index, (ix, name) in enumerate(name_index)
        if name == devices[ix].name
            qmax_cl[name, t] = @constraint(m, q_cl[name, t] <= devices[ix].maxreactivepower*devices[ix].scalingfactor.values[t])
        else
            error("Bus name in Array and variable do not match")
        end
    end
