function GenerationVariables(m::JuMP.Model, devices::Array{T,1}, time_steps) where T <: HydroGen
    on_set = [d.name for d in devices if d.status == true && !isa(d, HydroFix) ]
    t = 1:time_steps
...
"""
function PowerConstraints(m::JuMP.Model, phg::JuMP.JuMPArray{JuMP.Variable,2,Tuple{Array{String,1},UnitRange{Int64}}}, devices::Array{T,1}, time_periods::Int) where T <: HydroCurtailment
