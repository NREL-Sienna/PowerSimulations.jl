...
```variable[name, t] + varres[name, t] <= ts_data[2][ix]*ts_data[3][ix][t] ```

# LaTeX

`` x_t + x_t^{resr} \leq r^{val} r_t, \forall t ``
...
* var_name::Symbol : the name of the variable
"""
function reserve_device_timeseries_ub(canonical_model::CanonicalModel,
                              ts_data::Tuple{Vector{String}, Vector{Float64}, Vector{Vector{Float64}}},
                              cons_name::Symbol,
                              var_name::Symbol,
                              varres_name::Symbol)
...
@doc raw"""
    reserve_device_timeseries_param_ub(canonical_model::CanonicalModel,
                                    ts_data::Tuple{Vector{String}, Vector{Float64}, Vector{Vector{Float64}}},
                                    cons_name::Symbol,
                                    param_reference::UpdateRef,
                                    var_name::Symbol,
...
* var_name::Symbol : the name of the variable
"""
function reserve_device_timeseries_param_ub(canonical_model::CanonicalModel,
                                    ts_data::Tuple{Vector{String}, Vector{Float64}, Vector{Vector{Float64}}},
                                    cons_name::Symbol,
                                    param_reference::UpdateRef,
                                    var_name::Symbol,
    ...
    for t in time_steps, (ix, name) in enumerate(ts_data[1])
        param[name, t] = PJ.add_parameter(canonical_model.JuMPmodel, ts_data[3][ix][t])
        constraint[name, t] = JuMP.@constraint(canonical_model.JuMPmodel,
                                        variable[name, t] + varres[name, t] <= ts_data[2][ix]*param[name, t])
    end
