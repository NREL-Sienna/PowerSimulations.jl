function norm_two_constraint(ps_m::CanonicalModel,
    ...
    ps_m.constraints[cons_name] = JuMP.Containers.DenseAxisArray{JuMP.ConstraintRef}(undef, rating_data[1], lookahead)

    for t in lookahead, (ix,r) in enumerate(rating_data[1])
