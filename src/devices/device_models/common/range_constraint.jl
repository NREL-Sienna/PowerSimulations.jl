function device_range(ps_m::CanonicalModel,
    ...
    ps_m.constraints[cons_name] = JuMP.Containers.DenseAxisArray{JuMP.ConstraintRef}(undef, [r[1] for r in range_data], lookahead)

    for t in lookahead, r in range_data
...
function device_semicontinuousrange(ps_m::CanonicalModel,
    ...
    ps_m.constraints[ub_name] = JuMP.Containers.DenseAxisArray{JuMP.ConstraintRef}(undef, set_name, lookahead)
    ps_m.constraints[lb_name] = JuMP.Containers.DenseAxisArray{JuMP.ConstraintRef}(undef, set_name, lookahead)

    for t in lookahead, r in scrange_data
