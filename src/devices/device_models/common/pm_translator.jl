function get_branch_to_pm(ix::Int64, branch::PSY.PhaseShiftingTransformer)
    PM_branch = Dict{String,Any}(
        ...
        "f_bus"       => PSY.get_connectionpoints(branch).from |> PSY.get_number,
        "br_status"   => Float64(PSY.get_available(branch)),
        "t_bus"       => PSY.get_connectionpoints(branch).to |> PSY.get_number,
...
function get_branch_to_pm(ix::Int64, branch::PSY.HVDCLine)
    PM_branch = Dict{String,Any}(
        ...
        "pt"            => 0.0,
        "vt"            => PSY.get_connectionpoints(branch).to |> PSY.get_voltage,
        ...
        "vf"            => PSY.get_connectionpoints(branch).from |> PSY.get_voltage,
