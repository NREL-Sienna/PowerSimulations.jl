...
function activepower_constraints(ps_m::CanonicalModel,
    ...
    ts_data = [(d.name, d.maxactivepower * values(d.scalingfactor)) for d in devices]
...
function reactivepower_constraints(ps_m::CanonicalModel,
    ...
    for t in time_range, d in devices
            ...
            ps_m.constraints[:load_reactive_ub][d.name, t] = JuMP.@constraint(ps_m.JuMPmodel,  ps_m.variables[:Qel][d.name, t] == ps_m.variables[:Pel][d.name, t] * sin(atan((d.maxreactivepower/d.maxactivepower))))
