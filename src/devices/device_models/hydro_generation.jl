...
function activepower_variables(ps_m::CanonicalModel, devices::Array{H,1}, lookahead::UnitRange{Int64}) where {H <: PSY.HydroGen}

    add_variable(ps_m, devices, lookahead, :Phy, false, :nodal_balance_active)
...
function reactivepower_variables(ps_m::CanonicalModel, devices::Array{H,1}, lookahead::UnitRange{Int64}) where {H <: PSY.HydroGen}

    add_variable(ps_m, devices, lookahead, :Qhy, false, :nodal_balance_reactive)
...
function commitment_variables(ps_m::CanonicalModel, devices::Array{H,1}, lookahead::UnitRange{Int64}) where {H <: PSY.HydroGen}

    add_variable(ps_m, devices, lookahead, :on_hy, true)
    add_variable(ps_m, devices, lookahead, :start_hy, true)
    add_variable(ps_m, devices, lookahead, :stop_hy, true)
...
function activepower_constraints(ps_m::CanonicalModel,
                                  ...
                                  lookahead::UnitRange{Int64}) where {H <: PSY.HydroGen,
                                                                       D <: AbstractHydroDispatchForm,
                                                                       S <: PM.AbstractPowerFormulation}

    range_data = [(h.name, h.tech.activepowerlimits) for h in devices]

    device_range(ps_m, range_data, lookahead, hydro_active_range, :Phy)
...
function activepower_constraints(ps_m::CanonicalModel,
    ...
    device_timeseries_ub(ps_m, ts_data , lookahead, hydro_active_ub, :Phy)
...
function activepower_constraints(ps_m::CanonicalModel,
    ...
    device_timeseries_ub(ps_m, ts_data_ub , lookahead, :hydro_active_ub, :Phy)
    device_timeseries_lb(ps_m, ts_data_lb , lookahead, :hydro_active_lb, :Phy)
...
function reactivepower_constraints(ps_m::CanonicalModel,
    ...
    device_range(ps_m, range_data, lookahead, :hydro_reactive_range, :Qhy)
...
function activepower_constraints(ps_m::CanonicalModel,
    ...
    device_semicontinuousrange(ps_m, range_data, lookahead, :hydro_active_range, :Phy, :on_hy)
...
function reactivepower_constraints(ps_m::CanonicalModel,
    ...
    device_semicontinuousrange(ps_m, range_data , lookahead, :hydro_reactive_range, :Qhy, :on_hy)
...
# function nodal_expression(ps_m::CanonicalModel, devices::Array{R,1}, system_formulation::Type{S}, lookahead::UnitRange{Int64}) where {H <: PSY.Hydrogen, S <: PM.AbstractPowerFormulation}
#
...
# end
#
# function nodal_expression(ps_m::CanonicalModel, devices::Array{R,1}, system_formulation::Type{S}, lookahead::UnitRange{Int64}) where {H <: PSY.Hydrogen, S <: PM.AbstractActivePowerFormulation}
#
#     for t in lookahead, d in devices
#
#         _add_to_expression!(ps_m.expressions[:nodal_balance_active], d.bus.number, t, d.tech.rating * values(d.scalingfactor)[t])
