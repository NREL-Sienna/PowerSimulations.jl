...
function _get_time_series(device::Array{T}) where {T <: PSY.RenewableGen}
    ...
    for (ix,d) in enumerate(devices)
        names[ix] = d.name
        series[ix] = values(r.scalingfactor)*r.tech.installedcapacity
    end
...
function nodal_expression_param(ps_m::CanonicalModel,
    ...
    add_parameters(ps_m, ts_data, time_range, Symbol("Pre_$(eltype(devices))"), :var_active)
