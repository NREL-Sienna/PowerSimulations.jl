function construct_device!(
    ...
    add_variables!(psi_container, SubComponentReserveVariable, devices, PhysicalCoupling())
    add_variables!(
        psi_container,
...
function construct_device!(
    ...
    add_variables!(psi_container, SubComponentReserveVariable, devices, FinancialCoupling())
    add_variables!(
        psi_container,
        SubComponentActivePowerInVariable,
        devices,
...
function construct_device!(
    ...
    model::DeviceModel{T, D},
    ::Type{S},
) where {T <: PSY.HybridSystem, D <: TightCoupling, S <: PM.AbstractActivePowerModel}
    devices = get_available_components(T, sys)
...
function construct_device!(
    ...
    model::DeviceModel{T, D},
    ::Type{S},
) where {T <: PSY.HybridSystem, D <: TightCoupling, S <: PM.AbstractPowerModel}
    devices = get_available_components(T, sys)
