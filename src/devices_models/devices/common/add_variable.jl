...
function add_variable!(
    ...
    for t in time_steps, d in devices
        ...
        ub = get_variable_upper_bound(variable_type, service, d, optimization_container.settings)
        !(ub === nothing) && JuMP.set_upper_bound(variable[name, t], ub)

        lb = get_variable_lower_bound(variable_type, service, d, optimization_container.settings)
        !(lb === nothing) && !binary && JuMP.set_lower_bound(variable[name, t], lb)
