...
function device_range(psi_container::PSIContainer,
    ...
    constraint = add_cons_container!(psi_container, cons_name, set_name, time_steps)
    ...
    for r in range_data
          if abs(r[2].min - r[2].max) <= eps()
            @warn("The min - max values in range constraint with eps() distance to each other. Range Constraint will be modified for Equality Constraint")
                for t in time_steps
                    constraint[r[1], t] = JuMP.@constraint(psi_container.JuMPmodel, variable[r[1], t] == r[2].max)
                end
          else
                for t in time_steps
                    constraint[r[1], t] = JuMP.@constraint(psi_container.JuMPmodel, r[2].min <= variable[r[1], t] <= r[2].max)
                end
            end
    end
