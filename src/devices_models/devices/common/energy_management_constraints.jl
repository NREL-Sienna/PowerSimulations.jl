...
function energy_target!(
    ...
    varenergy = get_variable(container, var_types[1], U)
    varslack_up = get_variable(container, var_types[2], U)
    varslack_dn = get_variable(container, var_types[3], U)

    target_constraint = add_cons_container!(container, cons_type, U, name_index, time_steps)
...
function energy_target_param!(
    ...
    time_steps = get_time_steps(container)
    name_index = [get_component_name(d) for d in target_data]
    varenergy = get_variable(container, var_types[1], T)
    varslack_up = get_variable(container, var_types[2], T)
    varslack_dn = get_variable(container, var_types[3], T)

    container_target = add_param_container!(container, parameter, T, name_index, time_steps)
    param_target = get_parameter_array(container_target)
    multiplier_target = get_multiplier_array(container_target)
    target_constraint = add_cons_container!(container, cons_type, T, name_index, time_steps)

    for d in target_data, t in time_steps
        name = get_component_name(d)
        param_target[name, t] = PJ.add_parameter(container.JuMPmodel, d.timeseries[t])
